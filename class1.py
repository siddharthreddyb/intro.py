...
import time
number=random.randint(0, 35) #pick the number between 1 and 200

def intro():
    print("May I ask you for your name?")
    name=input() #asks for the name
    print(name + ", we are going to play a game. I am thinking of a number between 0 and 35")
    time.sleep(.2)
    print("Go ahead. Guess!")

def pick():
    guessesTaken = 0
    while guessesTaken < 3: #if the number of guesses is less than 6
        time.sleep(.10)
        ...
        try: #check if a number was entered
            ...
            if guess<=35 and guess>=0: #if they are in range
            ...
            if guess>35 or guess<0: #if they aren't in the range
                print("Silly Goose! That number isn't in the range!")
                time.sleep(.10)
                print("Please enter a number between 0 and 35")
